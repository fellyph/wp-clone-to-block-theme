...
8. **Motion profile recorded** — every section has `none`, `css-transition`, `css-keyframes`, `marquee`, `carousel`, `parallax`, `video`, `lottie`, or `scroll-triggered` from `analysis.motion` / `.capture/sections/<n>.json`.
